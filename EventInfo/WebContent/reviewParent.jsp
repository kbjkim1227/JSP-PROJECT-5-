	...
	<div class="row">
		...
		<div style="width: 100%; margin-bottom: 20px; display: flex;">
			...
			<div style="width: 25%; margin-right: 18px;">
				...
				color: black;" onclick="location.href='reviewParent.do'">
			</div>
		</div>
		...
		<div align="center">
			...
			%>
			<a href="reviewParent.do?page=1">처음으로</a>
			<%
			}
			if (pagination.getPage() > pagination.getPageSize()) {
			%>
			<a href="reviewParent.do?page=<%=pagination.getPreBlock()%>">&laquo;</a>
			<%
			}
			}
			%>
			...
			%>
			<a href="reviewParent.do?page=<%=i%>"><%=i%></a>
			<%
			}
			if (pagination.getTotalListCnt() != 0) {
			if (pagination.getStartPage() + pagination.getBlockSize() - 1 < pagination.getTotalPageCnt()) {
			%>
			<a href="reviewParent.do?page=<%=pagination.getNextBlock()%>">&raquo;</a>
			<%
			}
			if (pagination.getPage() != pagination.getTotalPageCnt()) {
			%>
			<a href="reviewParent.do?page=<%=pagination.getTotalPageCnt()%>">마지막으로</a>
			<%
			}
			}
			%>
