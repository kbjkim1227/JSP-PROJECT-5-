...
<div class="container">
	<div class="row">
		...
		<div style="width: 100%; margin-bottom: 20px; display: flex;">
			...
			<div style="width: 25%;">
				<input class="button1" type="button" value="강아지"style="background-color: #efefef; border-color: #ff7276; border-left: 2px solid #ff7276; border-right: 2px solid #ff7276; 
				color: black;" onclick="location.href='reviewDog.do'">
			</div>
			<div style="width: 25%;">
		...
		<div align="center">
			...
			%>
			<a href="reviewDog.do?page=1">처음으로</a>
			<%
			}
			if (pagination.getPage() > pagination.getPageSize()) {
			%>
			<a href="reviewDog.do?page=<%=pagination.getPreBlock()%>">&laquo;</a>
			<%
			}
			}
			%>
			...
			%>
			<a href="reviewDog.do?page=<%=i%>"><%=i%></a>
			<%
			}
			if (pagination.getTotalListCnt() != 0) {
			if (pagination.getStartPage() + pagination.getBlockSize() - 1 < pagination.getTotalPageCnt()) {
			%>
			<a href="reviewDog.do?page=<%=pagination.getNextBlock()%>">&raquo;</a>
			<%
			}
			if (pagination.getPage() != pagination.getTotalPageCnt()) {
			%>
			<a href="reviewDog.do?page=<%=pagination.getTotalPageCnt()%>">마지막으로</a>
			<%
			}
			}
			%>
