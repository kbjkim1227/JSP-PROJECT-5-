...
<div class="container">
	<div class="row">
		...
		<div style="width: 100%; margin-bottom: 20px; display: flex; color:grey;">
			<div style="width: 25%; ">
				<input class="button1" type="button"  style="background-color: #efefef; border-color: #ff7276; border-left: 2px solid #ff7276; border-right: 2px solid #ff7276; 
				color: black;" value="전체 보기" onclick="location.href='review.do'">
			</div>
			<div style="width: 25%;">
				<input class="button1" type="button" value="강아지"
					onclick="location.href='reviewDog.do'">
			</div>
			<div style="width: 25%;">
				<input class="button1" type="button" value="고양이"
					onclick="location.href='reviewCat.do'">
			</div>
			<div style="width: 25%; margin-right: 18px;">
				<input class="button1" type="button" value="기타 동물"
					onclick="location.href='reviewAnimal.do'">
			</div>
		</div>
		...
		<table
			...
			<tbody>
				...
				%>
				<tr style="font-weight: bold; font-family: IBMPlexSansKR-Regular;" >
					<td>
						<%
						if (review.getCategory().equals("dog"))
						%>강아지 <%
						if (review.getCategory().equals("cat"))
						%>고양이
						<%
						if (review.getCategory().equals("animal"))
						%>기타 동물
					</td>
		...
		<form action="reviewKeyword.do" method="get">
			<input class="sign_box" type="text" name="keyword">
			<input class="button2" type="submit" value="검색" color="black">
		</form>
		...
		<div align="center">
			...
			%>
			<a class="pg_no" href="review.do?page=1">처음으로</a>
			<%
			}
			if (pagination.getPage() > pagination.getPageSize()) {
			%>
			<a class="pg_no" href="review.do?page=<%=pagination.getPreBlock()%>">&laquo;</a>
			<%
			}
			}
			%>
			...
			%>
			<a class="pg_no" href="review.do?page=<%=i%>"><%=i%></a>
			<%
			}
			if (pagination.getTotalListCnt() != 0) {
			if (pagination.getStartPage() + pagination.getBlockSize() - 1 < pagination.getTotalPageCnt()) {
			%>
			<a class="pg_no" href="review.do?page=<%=pagination.getNextBlock()%>">&raquo;</a>
			<%
			}
			if (pagination.getPage() != pagination.getTotalPageCnt()) {
			%>
			<a class="pg_no" href="review.do?page=<%=pagination.getTotalPageCnt()%>">마지막으로</a>
			<%
			}
			}
			%>
