	...
	<div class="row">
		...
		<div style="width: 100%; margin-bottom: 20px; display: flex;">
			...
			<div style="width: 25%; margin-right: 18px;">
				<input class="button1" type="button" value="기타 동물" style="background-color: #efefef; border-color: #ff7276; border-left: 2px solid #ff7276; border-right: 2px solid #ff7276; 
				color: black;" onclick="location.href='reviewAnimal.do'">
			</div>
		</div>
		...
		<div align="center">
			...
			%>
			<a href="reviewAnimal.do?page=1">처음으로</a>
			<%
			}
			if (pagination.getPage() > pagination.getPageSize()) {
			%>
			<a href="reviewAnimal.do?page=<%=pagination.getPreBlock()%>">&laquo;</a>
			<%
			}
			}
			%>
			...
			%>
			<a href="reviewAnimal.do?page=<%=i%>"><%=i%></a>
			<%
			}
			if (pagination.getTotalListCnt() != 0) {
			if (pagination.getStartPage() + pagination.getBlockSize() - 1 < pagination.getTotalPageCnt()) {
			%>
			<a href="reviewAnimal.do?page=<%=pagination.getNextBlock()%>">&raquo;</a>
			<%
			}
			if (pagination.getPage() != pagination.getTotalPageCnt()) {
			%>
			<a href="reviewAnimal.do?page=<%=pagination.getTotalPageCnt()%>">마지막으로</a>
			<%
			}
			}
			%>
