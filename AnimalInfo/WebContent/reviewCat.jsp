	...
	<div class="row">
		...
		<div style="width: 100%; margin-bottom: 20px; display: flex;">
			...
			<div style="width: 25%;">
				...
				color: black;" onclick="location.href='reviewCat.do'">
			</div>
		...
		<div align="center">
			...
			%>
			<a href="reviewCat.do?page=1">처음으로</a>
			<%
			}
			if (pagination.getPage() > pagination.getPageSize()) {
			%>
			<a href="reviewCat.do?page=<%=pagination.getPreBlock()%>">&laquo;</a>
			<%
			}
			}
			%>
			...
			%>
			<a href="reviewCat.do?page=<%=i%>"><%=i%></a>
			<%
			}
			if (pagination.getTotalListCnt() != 0) {
			if (pagination.getStartPage() + pagination.getBlockSize() - 1 < pagination.getTotalPageCnt()) {
			%>
			<a href="reviewCat.do?page=<%=pagination.getNextBlock()%>">&raquo;</a>
			<%
			}
			if (pagination.getPage() != pagination.getTotalPageCnt()) {
			%>
			<a href="reviewCat.do?page=<%=pagination.getTotalPageCnt()%>">마지막으로</a>
			<%
			}
			}
			%>
